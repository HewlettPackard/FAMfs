...
#define pu_round_up_to_part(cpu, factor, parts, partition)		\
	({ unsigned int il = (cpu >> (factor)) % parts;			\
	   cpu += (((il>partition)?parts:0U)+partition-il) << (factor);	\
	   cpu; })
...
static int create_persistent_map(int map_id, int intl, char *name)
{
	...
	printf("%d: create_persistent_map:%d %s index[%u] interleave:%d\n",
	       md->mdhim_rank, map_id, name, id, intl);
	...
	for (e_sz = 1; e_sz <= 2; e_sz++) {
		...
		for (pass = 0; pass < RND_REPS; pass++) {
			...
			if (f_map_has_globals(m)) {
			    uint64_t cont_pu = e >> pu_factor;

			    /* avoid creation entry in foreign partition */
			    pu_round_up_to_part(cont_pu,
				m->geometry.intl_factor - pu_factor,
				m->parts, m->part);
			    e = cont_pu << pu_factor;
